...
def _authenticateChannel(channel, user, passw, client_id):
    ...
    """
    sec_stub = authentication_service_pb2.beta_create_Login_stub(channel)
    cred = authentication_service_pb2.LoginRequest(user_name=user,
                                                   password=passw,
                                                   client_id=client_id)
    res = sec_stub.LoginCheck(cred, TIMEOUT)
    return res
...
def Get_Site_Latest_Data():
    ...
    while(True):
        ...
        for i in range(1, n+1):
            "Get site IP"
            command_result = globals()['mgmt_S%s' % i].ExecuteOpCommand(op_oid1, TIMEOUT)
            result1 = None
            ...
            if result1.status == mgd_service_pb2.SUCCESS:
                if re.search (r"<object-value>(\d+.\d+.\d+.\d+)", str(result1.data)):
                    ...
                    command_result = globals()['mgmt_S%s' %i].ExecuteOpCommand(op_oid2, TIMEOUT)
                    result2 = None
                    for res in command_result:
                        if res.request_id == 2:
                            result2 = res
                    ...
                    if result2.status == mgd_service_pb2.SUCCESS:
                        if re.search (r"<object-value>(\d+)", str(result2.data)):
                            ...
                            "Get Site MODCOD"
                            command_result = globals()['mgmt_S%s' %i].ExecuteOpCommand(op_oid3, TIMEOUT)
                            result3 = None
                            for res in command_result:
                                if res.request_id == 3:
                                    result3 = res
...
def Apply_Cos(New_CIR, New_PIR, SITE_ID):
    ...
    for i in range(1, n-1):
        ...
        print "Pushing policer changes using JET AccessListPolicerReplace API"
        result = globals()['fw_R%s' % i].AccessListPolicerReplace(policer, TIMEOUT)
        if result.status == firewall_service_pb2.ACL_STATUS_EOK:
            ...
            ret = globals()['fw_R%s' %i].AccessListPolicerCounterGet(policerStats, TIMEOUT)
...
def Initial_Cos_Configs():
    ...
    for j in range(1, n-1):
        for i in range(1, n+1):
            ...
            result = globals()['fw_R%s' %j].AccessListPolicerAdd(policer, TIMEOUT)
            if result.status != firewall_service_pb2.ACL_STATUS_EOK:
            ...
            result = globals()['fw_R%s' %j].AccessListAdd(accessList, TIMEOUT)
            if result.status != firewall_service_pb2.ACL_STATUS_EOK:
                print "Something went wrong for R%s"%j
                print "Status received %s"%result.status
                exit()
            ...
            result = globals()['fw_R%s' %j].AccessListBindAdd(bindobj, TIMEOUT)
            if result.status != firewall_service_pb2.ACL_STATUS_EOK:
...
def Main():
    ...
    try:
        ...
        for i in range(1, n+1):
            creds = implementations.ssl_channel_credentials(open('{}.pem'.format(globals()['S%s_IP' %i])).read(), None, None)
            channel = implementations.secure_channel(globals()['S%s_IP' %i],int(PORT),creds)
            ...
            if res is False:
                return
            S_mgmt_stub = mgd_service_pb2.beta_create_ManagementRpcApi_stub(channel)
         
            globals()['mgmt_S%s' % i] = S_mgmt_stub

        "MX router connection and JET service handles"
        for i in range(1, n-1):
            creds = implementations.ssl_channel_credentials(open('{}.pem'.format(globals()['R%s_IP' %i])).read(), None, None)
            channel = implementations.secure_channel(globals()['R%s_IP' %i],int(PORT),creds)
            res = _authenticateChannel(channel, USER, PASSWORD, CLIENT_ID)
            print "Authentication "+('success' if res else 'failure')+' for '+globals()['R%s_IP' %i]
            if res is False:
                return
            
            R_fw_stub = firewall_service_pb2.beta_create_AclService_stub(channel)
            R_mgmt_stub = mgd_service_pb2.beta_create_ManagementRpcApi_stub(channel)
